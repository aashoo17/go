...
func Closure() {
	...
	//return b
	//somewhere returned b (i.e. closure ) could be called but till that time outer fuction main here is gone
	//so value of a should be gone - we kinda would be accessing a memory which should be gone
	//but go keeps it around
	//TODO: using main as returning closure is not good example may be confusing for other write seperate function to explain it
}
